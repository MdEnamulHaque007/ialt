...
class _PurchaseOrderPageState extends State<PurchaseOrderPage>
  ...
  void _calculateStats() {
    ...
    _totalPoValue = _pos.fold<double>(
      0,
      (total, po) => total + ((po['totalValue'] as num?)?.toDouble() ?? 0),
    );
  }
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    Text(
                                      'Total Quantity: ${NumberFormat('#,###').format(tempLines.fold<double>(0, (total, l) => total + (l['qty'] ?? 0)))}',
                                      style: const TextStyle(
                                    ...
                                    const SizedBox(height: 4),
                                    Text(
                                      'Total Value: \$${NumberFormat('#,###.##').format(tempLines.fold<double>(0, (total, l) => total + (l['totalValue'] ?? 0)))}',
                                      style: const TextStyle(
                                        fontSize: 18,
                                        fontWeight: FontWeight.bold,
                                        color: Colors.indigo,
                              ...
                              : () async {
                                  try {
                                    final totalQuantity = tempLines
                                        .fold<double>(
                                          0,
                                          (total, l) => total + (l['qty'] ?? 0),
                                        );
                                    final totalValue = tempLines.fold<double>(
                                      0,
                                      (total, l) => total + (l['totalValue'] ?? 0),
                                    );
