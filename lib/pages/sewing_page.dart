...
import '../services/sewing_service.dart';

class SewingPage extends StatefulWidget {
  const SewingPage({super.key});

  @override
  State<SewingPage> createState() => _SewingPageState();
}

class _SewingPageState extends State<SewingPage>
    with SingleTickerProviderStateMixin {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final SewingService _service = SewingService();

  List<Map<String, dynamic>> _sewings = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final snapshot = await _firestore.collection('sewing').get();
      _sewings = snapshot.docs.map((doc) {
      ...
      setState(() => _isLoading = false);
      _showSnackBar('Error loading sewing: $e', isError: true);
    }
  }

  List<Map<String, dynamic>> get _filtered {
    final q = _searchQuery.trim().toLowerCase();
    if (q.isEmpty) return _sewings;
    return _sewings.where((c) {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_editingId == null) {
        await _service.create(sewing);
        _showSnackBar('Sewing created successfully');
      } else {
        await _service.update(sewing);
        _showSnackBar('Sewing updated successfully');
      }
  ...
  Future<void> _delete(String id) async {
    try {
      ...
      _showSnackBar('Sewing deleted successfully');
      await _loadData();
    } catch (e) {
      _showSnackBar('Error: $e', isError: true);
    }
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          _editingId == null ? 'Add Sewing' : 'Edit Sewing',
  ...
  Widget _buildTextField({
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        labelText: label,
        prefixIcon: Icon(icon, size: 18, color: Colors.purple.shade700),
    ...
    return Container(
      ...
      child: Column(
        children: [
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    _buildStatChip('Records', '$totalItems', Colors.purple),
                    const SizedBox(width: 12),
                    ElevatedButton.icon(
                      onPressed: () => _showForm(),
                          ...
                          children: [
                            ...
                            ...paginated.asMap().entries.map((entry) {
                              ...
                              return Container(
                                color: (idx % 2 == 0)
                                    ? Colors.white
                                    : Colors.purple.shade50.withValues(
                                        alpha: 0.2,
                                      ),
                                child: Row(
                                  children: [
          ...
          if (!_isLoading && _sewings.isNotEmpty && filtered.isNotEmpty)
        ...
        child: Row(
          ...
          children: [
            IconButton(
              ...
              onPressed: () {
                final data = _sewings.firstWhere(
                  (x) => x['id']?.toString() == id,
                  orElse: () => {},
                );
  ...
  void _showDeleteDialog(String? id, int sl) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Delete Sewing?'),
