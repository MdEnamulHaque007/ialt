  ...
  Future<void> _loadDashboardStats() async {
    try {
      ...
      poValue = poSnapshot.docs.fold<double>(
        0,
        (total, doc) => total + (doc['totalValue']?.toDouble() ?? 0),
      );
      ...
      productionQty = prodSnapshot.docs.fold<double>(
        0,
        (total, doc) => total + (doc['qty']?.toDouble() ?? 0),
      );
      ...
      issueValue = issueSnapshot.docs.fold<double>(0, (total, doc) {
      ...
      exportValue = exportSnapshot.docs.fold<double>(0, (total, doc) {
        final qty = doc['quantity']?.toDouble() ?? 0;
        return total + qty * 10;
      });
      ...
      lcValue = lcSnapshot.docs.fold<double>(
        0,
        (total, doc) => total + (doc['master_lc_value']?.toDouble() ?? 0),
      );
